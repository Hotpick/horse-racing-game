...
## AI Traceability — Code Comments

Every file generated by Claude must include comments that make the AI's role and reasoning visible. This is a project requirement, not optional.

**File-level header** — add at the top of every `.ts` / `.vue` file:
```ts
// @ai-generated
// Implements: TASK-XXX — [task title]
// Spec ref:   SPEC.md §[section number]
```

**Algorithm comments** — any non-trivial logic must explain the "why", referencing the spec:
```ts
...
**Decision comments** — when choosing between approaches, note the reason:
```ts
// Using setInterval (not rAF) for engine ticks — easier to unit test with vi.useFakeTimers()
// See ARCHITECTURE.md §4.1
```

**TODO comments** — mark anything deferred to a later task:
```ts
// TODO(TASK-033): wire badge dot when Results update arrives on another tab
```
