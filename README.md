...
This project maintains explicit traces of AI involvement at every level:

**In documentation**
- `SPEC.md` — full game specification written with Claude
- `ARCHITECTURE.md` — all technical decisions documented with rationale
- `TASKS.md` — 40 atomic tasks generated by the `task-writer` skill
- `.claude/CLAUDE.md` — instructions Claude follows when generating code

**In source code** — every AI-generated file includes:
```ts
// @ai-generated
// Implements: TASK-XXX — task title
// Spec ref:   SPEC.md §section
```
Non-trivial logic includes inline comments referencing the spec and explaining the "why" behind algorithmic decisions.

**In git history** — every AI-assisted commit ends with:
```
